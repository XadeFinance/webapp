...
const HomePage = (props) => {
  ...
  const [latestTxs, setLatest] = useState([]);
  useEffect(() => {
    if (transactionHistory.length != 0) {
      if (transactionHistory.length < 5) {
        setLatest(transactionHistory.slice(0, transactionHistory.length));
      } else {
        setLatest(transactionHistory.slice(0, 5));
      }
    }
  }, [transactionHistory]);
      ...
      <div>
        ...
        <div className={home.txHistory}>
          ...
          <div className={home.transactions}>
            {latestTxs.map((transaction, index) => (
